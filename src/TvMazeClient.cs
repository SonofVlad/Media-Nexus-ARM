    ...
    internal static class TvMazeClient
    {
        public static Task<TvMazeLookup> LookupAsync(string show, int season, int firstEpisode, int count, CancellationToken token)
        {
            return Task.Run(() =>
            {
                ...
                var request = (HttpWebRequest)WebRequest.Create(url); request.UserAgent = "Media-Nexus-ARM/0.3.2 (https://github.com/SonofVlad/Media-Nexus-ARM)"; request.Accept = "application/json"; request.Timeout = 30000;
