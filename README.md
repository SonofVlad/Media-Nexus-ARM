...
## Media Nexus ARM in action

<p align="center">
  <img src="assets/Media-Nexus-ARM-running.png" alt="Media Nexus ARM concurrently ripping movies, TV series, and music across multiple optical drives">
</p>
...
- Always opens a manual title picker for Movie and TV Series discs, showing every title that meets the configurable minimum runtime.
- Prepares one video disc at a time in the background and securely caches completed MakeMKV title scans for faster selection on recognized discs.
...
No iTunes, MusicBrainz Picard, fre:ac installation, FFmpeg, Python, or API key is required. Media Nexus ARM stores its managed data under `%LOCALAPPDATA%\Media Nexus\ARM`, including a private portable fre:ac copy in `Tools\freac`. If FFmpeg is installed for optional or future Media Nexus features, ARM recognizes `ffmpeg.exe` in `%LOCALAPPDATA%\Media Nexus\Shared` or its `bin` subfolder and reports it in diagnostics.
